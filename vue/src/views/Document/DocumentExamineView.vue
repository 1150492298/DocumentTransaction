  ...
  <div class="DocumentExamineView">
    ...
    <div class="DocumentPostAddViewbody">
      <el-form :model="documentForm" label-width="auto"
        ...
        <el-form-item>
          <el-button type="primary" @click="documentApprove">批准</el-button>
          <el-button @click="documentReject">驳回</el-button>
        </el-form-item>
      </el-form>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    documentApprove() {
      const documentId = parseInt(this.$route.query.documentId)
      request.put('/document/documentApprove', {
        documentId: documentId,
        token: this.userCookie.token
      }).then(res => {
        if (res.code == '200') {
          this.$notify.success('已成功批准该公文!')
          this.$router.back()
        } else {
          this.$notify.error(res.msg)
        }
    ...
    documentReject() {
      const documentId = parseInt(this.$route.query.documentId)
      request.put('/document/documentReject', {
        documentId: documentId,
        token: this.userCookie.token
      }).then(res => {
        if (res.code == '200') {
          this.$notify.success('已驳回该公文!')
          this.$router.back()
        } else {
          this.$notify.error(res.msg)
        }
