<template>
  <div class="DocumentClassAdd">
    <AppIntroHeader HeaderClass="公文类别添加"></AppIntroHeader>
    <div class="DocumentClassAddViewbody">
      <el-form :model="documentClassForm" :rules="rules" ref="documentClassForm" label-width="auto"
        ...
        <el-form-item>
          <el-button type="primary" @click="updateDocumentClass('documentClassForm')">添加</el-button>
      ...
      this.$refs[documentClassForm].validate((valid) => {
        if (valid) {
          if(this.documentDepartmentRoleString.length===0){
            ...
            request.post('/documentClass/saveDocumentClass', this.documentClassForm).then(res => {
              if (res.code == '200') {
                this.$notify.success('新添公文流程成功')
                this.$router.back()
              } else {
