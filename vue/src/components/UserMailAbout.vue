  ...
  <div class="UserMailAbout">
    ...
    <div class="UserAboutFooter">
      ...
      <div class="UserAboutButton">
        <el-popconfirm title="确定要删除该通讯录？" @confirm="deleteAddressList" class="UserAboutButtonChild">
          <el-button size="mini" type="info" slot="reference" round>删除该通讯录
          </el-button>
        </el-popconfirm>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async deleteAddressList(){
      await  request.put('/user/deleteUserMailList', {
        deleteId:this.UserAbout.userId,
        token:this.userCookie.token
      }).then(res => {
        if (res.code == '200') {
          this.$notify.success('已从通讯录删除该用户')
          this.$parent.reload()
        } else {
